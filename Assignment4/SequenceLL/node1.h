    ...
    public:
	  ...
	  typedef double value_type;
	      ...
	      node(const value_type& init_data = value_type( ),
	          node* init_link = NULL){ data_field = init_data; link_field = init_link; }
...
// delete everything after the word iterator in the second line:
    ...
    {
    public:
    	node_iterator(node *initial = NULL) { current = initial; }
	    Item& operator *( ) const { return current->data( ); }
	    node_terator& operator ++( ) // Prefix ++
	    {
    ...
    class const_node_iterator:public std::iterator
    {
    public:
    	const_node_iterator(const node *initial = NULL) { current = initial; }
